...
if [ "${MAJOR_VERSION}" == "8" ]; then
  ...
  EXTRA_TAGS_BOOKWORM="${IMAGE_BASE}:${BRANCH_VERSION}-bookworm"
  build_multiplatform_image "${VERSION}/bookworm/Dockerfile" "$BOOKWORM_X86_LAST_LAYER_ID" "$BOOKWORM_ARM_LAST_LAYER_ID" "${IMAGE_BASE}:${VERSION}-bookworm" $EXTRA_TAGS_BOOKWORM

  EXTRA_TAGS_BOOKWORM_SLIM="${IMAGE_BASE}:${BRANCH_VERSION}-bookworm-slim ${IMAGE_BASE}:${BRANCH_VERSION}-slim"
  build_multiplatform_image "${VERSION}/bookworm-slim/Dockerfile" "$BOOKWORM_SLIM_X86_LAST_LAYER_ID" "$BOOKWORM_SLIM_ARM_LAST_LAYER_ID" "${IMAGE_BASE}:${VERSION}-bookworm-slim" "${IMAGE_BASE}:${VERSION}-slim" $EXTRA_TAGS_BOOKWORM_SLIM
